...
int SDScreen::run(const m5::touch_detail_t &t)
{

  if (M5.Touch.getCount() > 0)
  {
    auto t = M5.Touch.getDetail(0);
    if (t.wasPressed() && t.x < width / 2 && t.x > 0)
    {
      do_select_file(t.x, t.y);
    }
  }
  if (bexit != nullptr && state->displaySaver == DISPLAY_ACTIVE && bexit->handleTouch(t) )
  {
    return (0);
  }
  if (bup != nullptr && state->displaySaver == DISPLAY_ACTIVE &&  bup->handleTouch(t))
  {
    first_file = max(first_file - FILES_SCREEN, 0);
    drawSD();
  }
  if (bdown != nullptr && state->displaySaver == DISPLAY_ACTIVE &&  bdown->handleTouch(t))
  {
...
void SDScreen::drawFile(int file, bool selected)
{
  if (file >= 0 && file < n_files && file >= first_file && file < first_file + FILES_SCREEN)
  {
    ...
    if (selected)
    {
      ...
      M5.Display.fillRect(0, (file - first_file) * 30 + HEADER_SIZE, width / 2 + width / 6, 30, WHITE);
      M5.Display.drawString(sd_files[file], 10, HEADER_SIZE + 15 + (file - first_file) * 30);
    }
    else
    {
      M5.Display.setTextColor(WHITE);
      M5.Display.fillRect(0, (file - first_file) * 30 + HEADER_SIZE, width / 2 + width / 6, 30, BLACK);
      M5.Display.drawString(sd_files[file], 10, HEADER_SIZE + 15 + (file - first_file) * 30);
    }
  }
}
void SDScreen::drawSD()
{
  ...
  M5.Display.fillRect(0, 0, width / 2 + width / 6, height, BLACK);
  M5.Display.setTextDatum(CC_DATUM);

  M5.Display.setTextColor(CYAN);
  M5.Display.fillRect(0, 0, width / 2, HEADER_SIZE, BLUE);
  M5.Display.drawString("Registres", width / 4, HEADER_SIZE / 2);
...
void SDScreen::do_select_file(int tx, int ty)
{
  ...
  if (tx >= 0 && ty >= HEADER_SIZE)
  {
    int line = floor((ty - HEADER_SIZE) / 30) + first_file;
    if (line < n_files)
    {
