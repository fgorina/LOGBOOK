...
WindKalmanPolar::WindKalmanPolar() : lastTime_(0), initialised_(false) {
...
void WindKalmanPolar::step(float vx, float vy, uint64_t nowMs) {
  ...
  float qp = Q * dt2;     // position noise
  float qc = Q * dt;      // cross term  (use same Q for symmetry)
  float qv = Q_RATE * dt; // velocity noise (scaled by dt, not constant)

  PP[0][0] = P_[0][0] + dt * (P_[2][0] + P_[0][2]) + dt2 * P_[2][2] + qp;
  PP[0][1] = P_[0][1] + dt * (P_[2][1] + P_[0][3]) + dt2 * P_[2][3];
  PP[0][2] = P_[0][2] + dt * P_[2][2] + qc; // symmetric with PP[2][0]
  PP[0][3] = P_[0][3] + dt * P_[2][3];

  PP[1][0] = P_[1][0] + dt * (P_[3][0] + P_[1][2]) + dt2 * P_[3][2];
  PP[1][1] = P_[1][1] + dt * (P_[3][1] + P_[1][3]) + dt2 * P_[3][3] + qp;
  PP[1][2] = P_[1][2] + dt * P_[3][2];
  PP[1][3] = P_[1][3] + dt * P_[3][3] + qc; // symmetric with PP[3][1]

  PP[2][0] = P_[2][0] + dt * P_[2][2] + qc; // now == PP[0][2] ✓
  PP[2][1] = P_[2][1] + dt * P_[2][3];
  PP[2][2] = P_[2][2] + qv; // dt-scaled ✓
  PP[2][3] = P_[2][3];

  PP[3][0] = P_[3][0] + dt * P_[3][2];
  PP[3][1] = P_[3][1] + dt * P_[3][3] + qc; // symmetric with PP[1][3] ✓
  PP[3][2] = P_[3][2];
  PP[3][3] = P_[3][3] + qv; // dt-

  // After P update, catch any numerical runaway
  if (P_[0][0] > 1e6f || P_[1][1] > 1e6f) {
    state_[0] = vx;
    state_[1] = vy;
    state_[2] = 0;
    state_[3] = 0;
    P_[0][0] = P_[1][1] = 1.0f;
    P_[2][2] = P_[3][3] = 1.0f;
    P_[0][1] = P_[0][2] = P_[0][3] = 0;
    P_[1][0] = P_[1][2] = P_[1][3] = 0;
    P_[2][0] = P_[2][1] = P_[2][3] = 0;
    P_[3][0] = P_[3][1] = P_[3][2] = 0;
  }
...
WindKalmanResult WindKalmanPolar::update(float angle, float speed,
                                         uint64_t nowMs) {
  ...
  return WindKalmanResult{normNautic(ang), mag, ang_r, mag_r};
}
